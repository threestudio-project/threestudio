...
class SaverMixin:
    ...
    def get_rgb_image_(self, img, data_format, data_range, rgba=False):
        ...
        imgs = [img_ if img_.shape[-1] == nc else np.concatenate([img_, np.zeros((img_.shape[0], img_.shape[1], nc - img_.shape[2]), dtype=img_.dtype)], axis=-1) for img_ in imgs]
    ...
    def save_cubemap(self, filename, img, data_range=(0, 1), rgba=False):
        ...
        for start in range(0, img.shape[-1], 3):
            ...
            img_ = np.stack([self.get_rgb_image_(img_[i], 'HWC', data_range, rgba=rgba) for i in range(img_.shape[0])], axis=0)
            size = img_.shape[1]
