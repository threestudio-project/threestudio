...
class SpectralNorm:
    ...
    def mv(self, mat: torch.Tensor, vec: torch.Tensor):
        ...
        for i in range(0, mat.shape[0], chunk):
            out.append(torch.mv(mat[i : i + chunk], vec))
        ...
        if do_power_iteration:
            with torch.no_grad():
                for _ in range(self.n_power_iterations):
                    ...
                    v = normalize(self.mv(weight_mat.t(), u), dim=0, eps=self.eps, out=v)
                    u = normalize(self.mv(weight_mat, v), dim=0, eps=self.eps, out=u)
                if self.n_power_iterations > 0:
                    # See above on why we need to clone
                    u = u.clone(memory_format=torch.contiguous_format)
                    v = v.clone(memory_format=torch.contiguous_format)

        sigma = torch.dot(u, self.mv(weight_mat, v))
        weight = weight / sigma
        return weight
    ...
    def _solve_v_and_rescale(self, weight_mat, u, target_sigma):
        ...
        return v.mul_(target_sigma / torch.dot(u, self.mv(weight_mat, v)))
