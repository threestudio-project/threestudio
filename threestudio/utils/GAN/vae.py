...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        self.conv_in3 = torch.nn.Conv2d(z_channels+3,
                                       block_in,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if i_level != 0:
                up.upsample = Upsample(block_in, resamp_with_conv)
                up.rgb_conv = torch.nn.Conv2d(block_in+3, 3, kernel_size=3, stride=1, padding=1)
                up.rgb_cat_conv = torch.nn.Conv2d(block_in+3, block_in, kernel_size=3, stride=1, padding=1)
                curr_res = curr_res * 2
            self.up.insert(0, up) # prepend to get consistent order
