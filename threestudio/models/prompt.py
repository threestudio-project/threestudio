...
@threestudio.register('sjc-prompt-processor')
class SJCPromptProcessor(PromptProcessor):
    @dataclass
    class Config(PromptProcessor.Config):
        prompt: str = "a hamburger"
        negative_prompt: str = ""
    ...
    def configure(self) -> None:
        ...
        self.direction2idx = {d.name: i for i, d in enumerate(self.directions)}
        self.positive_prompts = [f"{d.prompt} {self.cfg.prompt} " for d in self.directions]
        self.negative_prompts = [f"{d.negative_prompt} {self.cfg.negative_prompt}" for d in self.directions]
        self.text_embeddings, self.uncond_text_embeddings = self.get_text_embeddings(
