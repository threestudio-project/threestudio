...
class Triplane(BaseImplicitGeometry):
    ...
    class Config(BaseImplicitGeometry.Config):
        plane_size: Tuple[int, int] = field(default_factory=lambda: (128, 128))
        n_feature_dims_plane: int = 32
        ...
        normal_type: Optional[
            str
        ] = "finite_difference"  # in ['finite_difference', 'finite_difference_laplacian']
    ...
    def configure(self) -> None:
        super().configure()
        self.n_plane = 3
        assert self.cfg.normal_type in [
            "finite_difference",
        ...
        self.planes: Float[Tensor, "3 C H W"] = nn.Parameter(
            torch.zeros(
                self.n_plane,
                self.cfg.n_feature_dims_plane,
                self.cfg.plane_size[0],
    ...
    def export(self, points: Float[Tensor, "*N Di"], **kwargs) -> Dict[str, Any]:
        ...
        points = points * 2 - 1  # convert to [-1, 1] for grid sample
        features = self.network(self.get_bilinear_feature(points, self.planes))[..., 1:]
        out.update(
            {
                "features": features,
            }
