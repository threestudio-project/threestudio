...
@dataclass
class SingleImage2DataModuleConfig:
    ...
    random_camera2: dict = field(default_factory=dict)
...
class SingleImage2DataBase:
    def setup(self, cfg, split):
        self.split = split
        self.rank = get_rank()
        self.cfg: SingleImage2DataModuleConfig = cfg
        ...
        if self.cfg.use_random_camera:
            ...
            random_camera_cfg2 = parse_structured(
                RandomCameraDataModuleConfig, self.cfg.get("random_camera2", {})
            )
            if split == "train":
                self.random_pose_generator2 = RandomCameraIterableDataset(
                    random_camera_cfg2
                )
            else:
                self.random_pose_generator2 = RandomCameraDataset(
                    random_camera_cfg2, split
                )
...
class SingleImage2IterableDataset(IterableDataset, SingleImage2DataBase, Updateable):
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        if self.cfg.use_random_camera:
            ...
            batch["random_camera2"] = self.random_pose_generator2.collate(None)
    ...
    def update_step(self, epoch: int, global_step: int, on_load_weights: bool = False):
        ...
        self.random_pose_generator2.update_step(epoch, global_step, on_load_weights)
...
class SingleImage2Dataset(Dataset, SingleImage2DataBase):
    def __init__(self, cfg: Any, split: str) -> None:
        super().__init__()
        self.setup(cfg, split)
...
@register("single-image2-datamodule")
class SingleImage2DataModule(pl.LightningDataModule):
    cfg: SingleImage2DataModuleConfig

    def __init__(self, cfg: Optional[Union[dict, DictConfig]] = None) -> None:
        super().__init__()
        self.cfg = parse_structured(SingleImage2DataModuleConfig, cfg)

    def setup(self, stage=None) -> None:
        if stage in [None, "fit"]:
            self.train_dataset = SingleImage2IterableDataset(self.cfg, "train")
        if stage in [None, "fit", "validate"]:
            self.val_dataset = SingleImage2Dataset(self.cfg, "val")
        if stage in [None, "test", "predict"]:
            self.test_dataset = SingleImage2Dataset(self.cfg, "test")
