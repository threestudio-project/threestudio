...
from threestudio.data.uncond import RandomCameraIterableDataset, RandomCameraDataset, RandomCameraDataModuleConfig
...
class SingleImageDataBase():
    def setup(self, cfg, split):
        ...
        rgba = cv2.resize(rgba, (self.cfg.width, self.cfg.height), interpolation=cv2.INTER_AREA).astype(np.float32) / 255.
        rgb = rgba[..., :3] * rgba[..., 3:] + (1 - rgba[..., 3:])
        self.rgb: Float[Tensor, "1 H W 3"] = torch.from_numpy(rgb).unsqueeze(0).contiguous().to(self.rank)
        self.mask: Float[Tensor, "1 H W 1"] = torch.from_numpy(rgba[..., 3:] > 0.5).unsqueeze(0).to(self.rank)
        print(f"[INFO] single image dataset: load image {self.cfg.image_path} {self.rgb.shape}")

        # load depth
        depth_path = self.cfg.image_path.replace('_rgba.png', '_depth.png')
        assert os.path.exists(depth_path)
        depth = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED)
        depth = cv2.resize(depth, (self.cfg.width, self.cfg.height), interpolation=cv2.INTER_AREA)
        self.depth: Float[Tensor, "1 H W 1"] = torch.from_numpy(depth.astype(np.float32) / 255.).unsqueeze(0).to(self.rank)
        print(f"[INFO] single image dataset: load depth {depth_path} {self.depth.shape}")
        ...
        c2w: Float[Tensor, "1 3 4"] = torch.cat([torch.stack([right, up, -lookat], dim=-1), camera_position[:, :, None]], dim=-1)
        ...
        directions_unit_focal = get_ray_directions(H=self.cfg.height, W=self.cfg.width, focal=1.0)
