...
@threestudio.register("dynamic-gaussian-splatting-du-system")
class DynamicGaussianSplattingDU(BaseLift3DSystem):
    @dataclass
    ...
    def training_step(self, batch, batch_idx):
        ...
        edit_image = cv2.imread(file_path.replace("images", "images_edit"))[
            :, :, ::-1
        ].copy()
        edit_image = (torch.FloatTensor(edit_image) / 255).unsqueeze(0)
        self.edit_frames[batch_index] = edit_image
        ...
        out = self(batch)
        os.makedirs(
            os.path.dirname(file_path).replace("images", "images_render"), exist_ok=True
        )
        save_img = out["refine"].permute(1, 2, 0).clamp(0, 1).detach().cpu()
        cv2.imwrite(
            file_path.replace("images", "images_render"),
            (save_img.numpy() * 255).astype(np.uint8)[:, :, ::-1],
        )
        dataroot = os.path.dirname(os.path.dirname(file_path))
        status_json = {"global_step": self.global_step}
        json.dump(status_json, open(os.path.join(dataroot, "status.json"), "w"))
