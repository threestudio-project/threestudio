...
@threestudio.register("dynamic-gaussian-splatting-gan-reconstruct-system")
class DynamicGaussianSplattingGANReconstruction(BaseLift3DSystem):
    @dataclass
    ...
    def training_step(self, batch, batch_idx):
        ...
        refine_gt_rgb = gt_rgb.permute(0, 3, 1, 2)[
            :, :, patch_y : patch_y + S, patch_x : patch_x + S
        ]
        refine_size = out["refine"].shape[-1]
        refine_gt_rgb = torch.nn.functional.interpolate(
            refine_gt_rgb, (refine_size, refine_size), mode="bilinear"
        )
        ...
        guidance_out = {
            ...
            "loss_G_l1": torch.nn.functional.l1_loss(out["refine"], refine_gt_rgb[0]),
            "loss_G_p": self.perceptual_loss(
                out["refine"].unsqueeze(0).contiguous(),
                refine_gt_rgb.contiguous(),
            ).mean(),
