...
class Zero123(BaseLift3DSystem):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.save_image_grid(
            f"training_vid/it{self.true_global_step}-{batch['index'][0]}.png",
            [
                {
                    "type": "rgb",
                    "img": out["comp_rgb"][0],
        ...
        if getattr(self.cfg.freq, "actual_val", False):
            if self.true_global_step % getattr(self.cfg.freq, "actual_val") != 0:
                return
    ...
    def on_validation_epoch_end(self):
        ...
        self.save_img_sequence(
            ...
            save_format="mp4",
            fps=30,
            name=f"it{self.true_global_step}-training_vid",
            step=self.true_global_step,
        )
        for file in os.path.join(self.get_save_dir(), filestem, "*.png"):
            os.remove(file)
        if getattr(self.cfg.freq, "actual_val", False):
            if self.true_global_step % getattr(self.cfg.freq, "actual_val") != 0:
                return
        ...
        shutil.rmtree(os.path.join(self.get_save_dir(), filestem))
